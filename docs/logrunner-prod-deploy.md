...
For a first deploy, set `OPENAI_API_KEY` in `.env.prod` or store it in the macOS Keychain as `DEMO_OPENAI_API_KEY`, then provide MultiBaas API keys through `MULTIBAAS_BACKENDS_FILE` or the single-backend `MULTIBAAS_API_KEY` fallback. For non-OpenAI providers that expose an OpenAI-compatible API, prefer `OPENCODE_API_KEY` so the credential is not confused with an official OpenAI key. The local deploy script defaults to `OPENAI_API_KEY_SOURCE=keychain`, so the Keychain item wins over an ignored `.env.prod` value unless `OPENAI_API_KEY` was explicitly exported in the shell. To use a different Keychain item name, set `OPENAI_API_KEY_KEYCHAIN_ITEM`; to force the env-file key, set `OPENAI_API_KEY_SOURCE=env`. The remote deploy script redacts raw env-file key values back out of `/opt/logrunner-prod/shared/logrunner.env` after installing them into OneCLI. Codex uses OpenAI's websocket Responses path, which is not covered by the current OneCLI injection flow, so the official OpenAI demo key is also written to `/opt/logrunner-prod/shared/nanoclaw.env` with mode `600`. Keep raw keys out of NanoClaw `container.json`; MultiBaas credentials still flow through OneCLI placeholder bearer tokens.
...
2. Use `NANOCLAW_REF=deploy/logrunner-prod` for the isolated production branch.
3. For a frozen judging release, either set `NANOCLAW_REF` to the tested commit SHA or move the lightweight prod branch/tag to that commit.
