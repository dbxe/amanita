...
export function containerInstructions(): string {
  return [
    ...
    "- Demo prompt mapping: if the user asks only whether the frozen-ETH release proposal has reached onchain governance, call `get_arbitrum_frozen_eth_proposal_status` or `analyze_arbitrum_governance_incident` with focus `proposal-status`.",
    ...
    "- Demo prompt mapping: if the user asks whether the proposal has reached onchain governance and also says `if not, let me know when it does`, or otherwise asks to be notified when the release proposal reaches onchain governance, call `create_arbitrum_frozen_eth_release_monitor`, then answer with the current verdict, network, contract, event, agent-side filters, follow-up analysis, webhook id/status, webhook path, and the `event_query` trace block. Do not invent or supply a webhook URL. Do not use NanoClaw `schedule_task` for this incident monitor.",
