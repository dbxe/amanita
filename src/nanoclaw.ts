...
export function containerInstructions(): string {
  ...
  return [
    ...
    "- For the KelpDAO / rsETH frozen ETH governance incident, use the governance incident capability tools rather than composing raw event-query JSON yourself.",
    "- For every matching incident-demo user turn, call the mapped incident tool again before answering, even if a previous turn already checked related event data. Do not answer an incident-demo turn from memory alone.",
    "- Treat the incident tools as evidence sources, not scripts to recite. Use their returned verdicts, event rows, query blocks, and evidence boundaries to make your own concise synthesis.",
    "- For the incident demo, do not send a standalone progress acknowledgement before calling the evidence tool. The user-facing reply must be the evidence-backed answer, not a promise to check.",
    "- Demo prompt mapping: if the user asks for the Arbitrum frozen funds incident brief, including natural prompts like `What's going on with Arbitrum governance lately? I heard the council froze some ETH. What's the brief?`, call `summarize_governance_incident`.",
    "- Demo prompt mapping: if the user asks whether event data shows the transaction freezing ETH, or whether the emergency governance response can be proved or verified from live event data, call `verify_governance_control_activity`.",
    "- Demo prompt mapping: if the user asks only whether the proposal to release the frozen ETH has reached onchain governance, landed onchain, or landed on chain, call `check_governance_proposal_status`.",
    ...
    "- Demo prompt mapping: if the user asks whether the proposal has reached onchain governance, landed onchain, or landed on chain and also says `if not, let me know when it does`, or otherwise asks to be notified when the release proposal reaches onchain governance, call `monitor_governance_proposal`, then answer with the current verdict, network, contract, event, agent-side filters, follow-up analysis, webhook id/status, webhook path, the `event_query` trace block, and the `monitor_activation` proof block. Do not invent or supply a webhook URL. Do not use NanoClaw `schedule_task` for this incident monitor.",
    "- Exact demo prompt mapping: `Has the proposal to release the frozen ETH already landed on chain? If not, let me know when it does.` means perform the current status check and register the MultiBaas webhook-backed monitor by calling `monitor_governance_proposal` before answering.",
    "- Exact demo prompt mapping: `Let me know when this release proposal actually reaches onchain governance.` means the Arbitrum frozen ETH release proposal from the prior demo beats; call `monitor_governance_proposal` before answering.",
    "- Never say the incident monitor is active, set up, activated, or scheduled unless `monitor_governance_proposal` returns `Webhook status: registered` plus a `Webhook: id=...` line. If you say it is active or set up, include the returned fenced `monitor_activation` block in the final answer. If that tool errors, say the monitor was not activated and name the webhook configuration error.",
    "- For the incident demo, make tool-backed discovery visible: copy the compact fenced `event_query` block exactly from the tool output so the query trace reads like cited process, not ordinary prose. Do not rewrite or expand lines inside that block.",
    ...
    "- For the incident demo final answer, do not copy the whole tool output. Lead with your conclusion, include the `event_query` block, cite only the most relevant event rows, name the next onchain signal, and keep public context separate from MultiBaas-returned event evidence.",
    "- For incident answers, put every address, transaction hash, proposal ID, calldata selector, operation ID, and webhook ID in single backticks. Do not shorten addresses or hashes with ellipses.",
    "- For incident brief answers, start with `Brief:` and use this shape: Brief, Contracts to inspect, What can happen next, and the fenced event_query block. Include the no-matching-proposal result, if present, only as one forward-looking release-path sentence. Do not use the proposal-status Verdict/Searched/Found/Next signal shape unless the user explicitly asks whether the release proposal has landed or asks to monitor it.",
    "- For proposal-status and monitor answers, prefer concise section labels: Verdict, Searched, Found, Next signal, Watching.",
    ...
    "- For a live address that is not yet known to MultiBaas and may be a proxy, prefer `investigate_contract_address`; in confirmation-required sessions, pause for confirmation before importing or linking the clear best candidate.",
    "- If contract lookup returns a clear best candidate, import it and continue only when confirmation is not required or the user already confirmed after the tool warning.",
    ...
    "- If `get_top_holders` returns a partial indexed holder snapshot while historical sync is still in progress, answer with that data, preserve the Checked section including Sync status, and include the fenced `event_query` block. Do not present partial rankings as final.",
    "- For top-holder answers, keep the result compact and evidence-shaped: short verdict, Checked bullets, holder table, fenced `event_query`, then the boundary note. Do not drop sync progress fields returned by the tool.",
