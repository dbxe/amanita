...
export function containerInstructions(): string {
  return [
    ...
    "- Mandatory routing: if the user mentions KelpDAO, rsETH, frozen ETH, frozen funds, the 30,765 ETH freeze, the 0x0000000000000000000000000000000000000DA0 address, or the Arbitrum release proposal, call the Arbitrum governance incident tool surface before answering. Use `analyze_arbitrum_governance_incident` or the dedicated frozen-ETH aliases. Do not answer those prompts from memory.",
    ...
    "- For the KelpDAO / rsETH frozen-ETH governance incident demo, use `analyze_arbitrum_governance_incident` or a dedicated frozen-ETH alias rather than composing raw event-query JSON yourself.",
    ...
    "- Demo prompt mapping: if the user asks for the Arbitrum frozen-funds incident brief, including natural prompts like `Anything weird with Arbitrum governance lately? I heard the council froze some ETH`, call `get_arbitrum_frozen_eth_governance_brief` or `analyze_arbitrum_governance_incident` with focus `brief`.",
    "- Demo prompt mapping: if the user asks whether the emergency governance response can be proved or verified from live event data, including live onchain event data, call `verify_arbitrum_frozen_eth_emergency_response` or `analyze_arbitrum_governance_incident` with focus `verify-freeze`.",
    "- Demo prompt mapping: if the user asks only whether the frozen-ETH release proposal has reached onchain governance, landed onchain, or landed on chain, call `get_arbitrum_frozen_eth_proposal_status` or `analyze_arbitrum_governance_incident` with focus `proposal-status`.",
    ...
    "- Demo prompt mapping: if the user asks whether the proposal has reached onchain governance, landed onchain, or landed on chain and also says `if not, let me know when it does`, or otherwise asks to be notified when the release proposal reaches onchain governance, call `create_arbitrum_frozen_eth_release_monitor`, then answer with the current verdict, network, contract, event, agent-side filters, follow-up analysis, webhook id/status, webhook path, the `event_query` trace block, and the `monitor_activation` proof block. Do not invent or supply a webhook URL. Do not use NanoClaw `schedule_task` for this incident monitor.",
    "- Exact demo prompt mapping: `Has the frozen-ETH release proposal reached onchain governance yet? If not, let me know when it does.` means perform the current status check and register the MultiBaas webhook-backed monitor by calling `create_arbitrum_frozen_eth_release_monitor` before answering.",
