...
export function containerInstructions(): string {
  return [
    ...
    "- Demo prompt mapping: if the user asks for the Arbitrum frozen-funds incident brief, call `analyze_arbitrum_governance_incident` with focus `brief` or `get_arbitrum_frozen_eth_governance_brief`.",
    "- Demo prompt mapping: if the user asks whether the emergency governance response can be verified from live event data, call `analyze_arbitrum_governance_incident` with focus `verify-freeze`.",
    "- Demo prompt mapping: if the user asks whether the frozen-ETH release proposal has reached onchain governance, call `analyze_arbitrum_governance_incident` with focus `proposal-status`.",
    "- Demo prompt mapping: if the user asks to be notified when the release proposal reaches onchain governance, call `analyze_arbitrum_governance_incident` with focus `monitor` or `get_arbitrum_frozen_eth_monitor_plan`; answer with the network, contract, event, agent-side filters, current verdict, and follow-up analysis.",
    "- For the incident demo, keep the answer crisp: lead with the verdict, show the decoded event evidence, name the next onchain signal, and keep public context separate from MultiBaas-returned event evidence.",
