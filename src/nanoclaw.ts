...
export function containerInstructions(): string {
  return [
    ...
    "- For live event-query turns, if `send_message` is available, you must use a two-step cadence: first send one short progress message saying what you are about to check, then call the evidence tool, then send the final answer. Use at most one progress message per user turn.",
    "- A progress message only counts if it is sent by actually calling the NanoClaw `send_message` tool. Do not output only the progress note as your final answer. If you cannot call `send_message`, skip the progress note and call the evidence tool directly.",
    "- Good progress messages are specific and short: 'I’ll check the Core Governor ProposalCreated stream for Kelp / rsETH markers.' or 'I’ll check the L1/L2 timelock and upgrade-executor event streams.'",
    "- Demo prompt mapping: if the user asks for the Arbitrum frozen-funds incident brief, call `get_arbitrum_frozen_eth_governance_brief` or `analyze_arbitrum_governance_incident` with focus `brief`.",
    ...
    "- Demo prompt mapping: if the user asks to be notified when the release proposal reaches onchain governance, call `get_arbitrum_frozen_eth_monitor_plan` or `analyze_arbitrum_governance_incident` with focus `monitor`; answer with the network, contract, event, agent-side filters, current verdict, and follow-up analysis.",
